...
class EventosRepository @Inject constructor(private val eventosDao: EventosDao) {
    ...
    fun getAllEventos():List<Evento>{
        ...
        return entities.map {
            Evento(id = it.id, titulo = it.titulo, descrip = it.descrip, fecha = it.fecha, prioridad = it.prioridad)
        }
    }

    suspend fun deleteEvento(evento: Evento){
        val entity=EventoEntity(id = evento.id, titulo = evento.titulo, descrip = evento.descrip, fecha = evento.fecha, prioridad = evento.prioridad)
        eventosDao.deleteEvento(entity)
    }

    suspend fun insertEvento(evento: Evento){
        val entity = EventoEntity(titulo = evento.titulo, descrip = evento.descrip, fecha = evento.fecha, prioridad = evento.prioridad)
